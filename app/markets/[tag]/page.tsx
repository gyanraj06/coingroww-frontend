...
export default async function MarketsTagPage({ params }: { params: { tag: string } }) {
    const { tag } = params;
    const { latest, editorsPicks, trending } = await getData('Markets', tag);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-8 space-y-8">
                    <div className="flex items-center justify-between border-b border-[#1f1f1f] pb-4">
                        <h2 className="text-2xl font-bold text-white border-l-4 border-green-500 pl-4 uppercase">
                            {displayTag} Markets
                        </h2>
                    </div>
